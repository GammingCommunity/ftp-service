...
class FileHelper
{
	 ...
	 */
	public static function download(string $diskName, ?string $fileName, string $defaultFileName = null, bool $browserAutoDownload = true): ?BinaryFileResponse
	{
		$result = null;
	 ...
	 * @return FtpServiceResponse
	 */
	public static function store(string $diskName, ?UploadedFile $file, ?array $fileExtensions = null, ?string $fileName = null, int $randomNameLength = 128): FtpServiceResponse
	{
		$response = new FtpServiceResponse(FtpServiceResponseStatus::SUCCESSFUL);
		...
		if (self::checkExistUploadingFile($file)) {
			if (self::checkFileExtension($file, $fileExtensions)) {
				if ($fileName == null) {
					...
					$randomName = Str::random($randomNameLength) . '.' . $file->extension();
					while (self::exist($diskName, $randomName)) {
						$randomName = Str::random($randomNameLength) . '.' . $file->extension();
					}
					//
					$result = $file->storeAs('', $randomName, $diskName);
					if ($result) {
	 ...
	 * @return bool
	 */
	public static function checkFileExtension(UploadedFile $file, ?array $fileExtensions = null): bool
	{
	...
	/**
	 * Return true if it is UploadedFile type, and file size > 0
	 * 
	 * @param UploadedFile|null $file
	 * @return bool
	 */
	public static function checkExistUploadingFile(?UploadedFile $file): bool
	{
