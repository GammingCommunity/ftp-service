...
class FileHelper
{
	...
	public static function changeFile(string $diskName, ?UploadedFile $newFile, string $oldFileName): FtpServiceResponse
	{
		if (self::existFile($diskName, $oldFileName)) {
			...
			if ($response->code === FtpServiceResponseStatus::SUCCESS) {
	...
	{
		$response = new FtpServiceResponse(FtpServiceResponseStatus::SUCCESS);

		if ($fileName && Storage::disk($diskName)->exists($fileName)) {
			$response->code = Storage::disk($diskName)->delete($fileName) ? FtpServiceResponseStatus::SUCCESS : FtpServiceResponseStatus::SOMETHING_ELSE;
			if ($response->code === FtpServiceResponseStatus::SOMETHING_ELSE) {
				$thisFile = __FILE__;
				$thisLine = __LINE__;
	...
	public static function storeFileForModel(string $diskName, ?UploadedFile $file, Model $model, string $fieldName, ?array $fileExtensions = null): FtpServiceResponse
	{
		...
		if ($response->code === FtpServiceResponseStatus::SUCCESS) {
			...
			if (!$model->save()) {
				$response->code = FtpServiceResponseStatus::SOMETHING_ELSE;
